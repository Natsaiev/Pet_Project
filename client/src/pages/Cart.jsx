...
import { getTotal, clearCart } from "../store/cartSlice"; // Импортируйте новый редюсер
...
export const Cart = () => {
    ...
    const handleOrder = () => {
        setOpen(true);
        dispatch(clearCart()); // Очистите корзину
    };
                ...
                <Stack direction="row" alignItems="center" mb={5}>
                    ...
                    <Stack direction="row" alignItems="center">
                        <Box sx={{ flexGrow: 1, ml: 4 }}>
                            <Divider sx={{ borderColor: "#ddd", borderBottomWidth: 2, width: "1100px", mr: 1 }} />
                        </Box>
                        <Link to="/">
                            <LinkBorderBtn sx={{ width: "100%" }}>Back to the store</LinkBorderBtn>
                        </Link>
                    </Stack>
                </Stack>
                {productsInCart.length > 0 ? (
                    <Stack container direction={{ xxs: "column", xs: "column", sm: "column", md: "row", lg: "row", xl: "row" }} spacing={4} sx={{ gap: { xxs: 0, xs: 0, sm: 0, md: 4, lg: 4, xl: 8 } }}>
                        ...
                        <Box size="auto" mb={5} sx={{ minWidth: { md: 370, lg: 480, xl: 548 } }}>
                            <OrderDetails items={productsInCart.length} total={total} handleClickOpen={handleOrder} /> {/* Обновите функцию обработки */}
                        </Box>
                    </Stack>
                ) : (
                    <Stack mb={8}>
